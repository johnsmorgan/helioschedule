...
parser.add_argument('coarse_chans', help='comma-separated list of coarse chan ranges: e.g. 121-132,157-180')
...
for c, chan_str in enumerate(coarse_chans):
    ...
    for i in sweetspots:
        ...
        d_chan = np.average(df_in['beams'][i, low_index:low_index+n_chan, :, :, :], axis=1, weights=df_in['beams'][i, low_index:low_index+n_chan, :, :, :]**2)
        ...
        broadnesss[c][i] = np.nansum(d*np.cos(np.radians(df_in['alt_scale'][...])).reshape((-1, 1)))
